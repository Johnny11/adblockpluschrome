...
function checkPotentialPopup(tabId, popup)
{
  ...
  if (filter instanceof BlockingFilter) {
    ...
    let request = {
      ...
      subscriptions: filter.subscriptions.reduce(function(map, entry){ map[entry.title] = {version: entry.version, lastCheck: entry.lastCheck,lastSuccess: entry.lastSuccess, title: entry.title, url: entry.url, homepage: entry.homepage, status: entry.downloadStatus}; return map }, {}),
    };
    
    chrome.tabs.sendMessage(popup.sourcePage.id, {log: {data: request}});
